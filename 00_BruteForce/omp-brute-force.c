...
#pragma omp parallel default(none) shared(KEY_LEN, msglen,n, found, CHECK_LEN, check, enc )
{
  ...
  assert(out != NULL);
  for (int k = my_start; k < my_end && !found; k++)     // devo stare attento alla condizione del ciclo, qui devo fare a mano per
                                                        //perche ce l'and 
  {
    ...
    { // controllo i primi 10 caratteri
      printf("Key found: %s\n", key);
