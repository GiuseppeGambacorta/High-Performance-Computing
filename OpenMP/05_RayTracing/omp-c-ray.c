...
void render(int xsz, int ysz, pixel_t *fb, int samples)
{
    ...
    #pragma omp parallel for collapse(2) default(none) shared(samples,fb,xsz,ysz)
    for (long j=0; j<ysz; j++) {
        for (long i=0; i<xsz; i++) {
            double r, g, b;
            r = g = b = 0.0;
           // #pragma omp parallel for default(none) shared(i,j,r,g,b,samples)
